...
//                 icon: 'info',
//                 title: 'Note',
//                 text: 'This patient has no prior history.',
//                 });
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const fullNameInput = document.getElementById('full-name'); // Corrected to getElementById
    const accessToken = localStorage.getItem('access_token'); // Replace with actual token retrieval method if different
    ...
    function calculateAge() {
        ...
        if (dobString) {
            ...
            ageInput.value = ''; // Clear age if DOB is cleared
        }
    }

    // Add event listener for manual DOB changes
    if (dateOfBirthInput && ageInput) {
        dateOfBirthInput.addEventListener('change', calculateAge);
    }

    // Next button logic
    if (nextButton && patientInfoDiv && phoneNumberInput && fullNameInput) {
        nextButton.addEventListener("click", function () {
            ...
            .then(data => {
                ...
                if (data.exists) {
                    ...
                    // Set patient details
                    phoneNumberInput.value = patient.phone_number;
                    fullNameInput.value = patient.full_name;
                    document.getElementById("husband-name").value = patient.husband_name;
                    document.getElementById("husband-phone").value = patient.husband_phone;
                    // document.getElementById("couple-no").value = patient.couple_no;
                    document.getElementById("nid-number").value = patient.nid_number;
                    dateOfBirthInput.value = patient.date_of_birth; // Set date of birth from API
                    
                    // !!! IMPORTANT: Manually trigger the change event to update the age field
                    // This ensures the age is calculated even if the DOB input isn't directly interacted with by the user.
    ...
    // Location dropdowns logic (existing from your code)
    const divisionSelect = document.getElementById("division");
    const districtSelect = document.getElementById("district");
    const upazilaSelect = document.getElementById("upazila");
    const unionSelect = document.getElementById("union");
