        ...
        .then(data => {
            ...
            if (data.exists && data.patient) {
                ...
                const patientImage = patient.image
                    ? `<img src="${patient.image}" alt="Patient Image" class="rounded-circle img-fluid border border-secondary mb-3 shadow-sm" style="width: 120px; height: 120px; object-fit: cover;">`
                    : `<div class="rounded-circle bg-secondary d-flex align-items-center justify-content-center text-white mb-3 shadow-sm" style="width: 120px; height: 120px; font-weight: 600; font-size: 0.9rem;">No Image</div>`;

                // Generate ANC Schedule List
                let ancScheduleHTML = `<h5 class="mb-3 text-primary fw-semibold">ANC Schedules</h5>`;
                if (patient.anc_schedules && patient.anc_schedules.length > 0) {
                    ancScheduleHTML += `<ul class="list-group shadow-sm rounded">`;
                    patient.anc_schedules.forEach(schedule => {
                        ...
                        if (schedule.status === "Scheduled") {
                            statusBadge = "warning";
                            actionButton = `<button class="btn btn-outline-primary btn-sm create-checkup" 
                                                data-id="${schedule.id}" 
                                                data-anc-id="${schedule.id}"
                                                data-date="${schedule.anc_date}">
                                                <i class="bi bi-plus-circle me-1"></i>Create Checkup
                                            </button>`;
                        }
                        if (schedule.status === "Completed") {
                            statusBadge = "success";
                            actionButton = `<a href="/show_report/${schedule.id}" class="btn btn-success btn-sm">
                                                <i class="bi bi-file-earmark-text me-1"></i>Show Report
                                            </a>`;
                        }
                        ...
                        ancScheduleHTML += `
                            <li class="list-group-item d-flex justify-content-between align-items-center flex-wrap border-bottom-1 border-primary border-dotted" style="">
                                <div>
                                    <strong>${schedule.anc_date || "No Date"}</strong>
                                    <span class="badge bg-${statusBadge} ms-2">${schedule.status}</span>
                                </div>
                    ...
                    ancScheduleHTML += `<p class="text-muted fst-italic">No ANC schedules found.</p>`;
                }

                // Render Patient Info + ANC Schedule
                patientInfoDiv.innerHTML = `
                    <div class="row bg-white shadow rounded p-4 gap-4 gap-md-0">
                        <div class="col-12 col-md-5 text-center">
                            ${patientImage}
                            <h4 class="fw-bold mb-1">${patient.full_name || "N/A"}</h4>
                            <p class="text-muted mb-0"><i class="bi bi-telephone me-1"></i>Phone: ${patient.phone_number || "N/A"}</p>
                        </div>
                        <div class="col-12 col-md-7 border-start border-md-start-0 border-top border-md-top-0 pt-3 pt-md-0 ps-md-4">
                            ${ancScheduleHTML}
                        </div>
                    </div>
                `;
